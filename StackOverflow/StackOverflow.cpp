...
Post* StackOverflow::addQuestion(const std::string& userId, const std::string& content, const std::vector<std::string>& tags){
...
Post* StackOverflow::addAnswer(const std::string& userId, const std::string& questionId, const std::string &content){
    if (!findUser(userId)) return nullptr; 
...
Comment* StackOverflow::addComment(const std::string& userId, const std::string& postId, const std::string &content) {
    if (!findUser(userId)) return nullptr; 
...
std::vector<Post*> StackOverflow::searchQuestions(const std::string& tag) const {
    ...
    for (const auto& post: posts){
        if (post->getType() == PostType::QUESTION){
            ...
            if (std::find(tags.begin(), tags.end(), tag) != tags.end()) results.push_back(post); 
        }
    }
...
User* StackOverflow::findUser(const std::string& userId) const {
    auto it = std::find_if(users.begin(), users.end(), [userId](User* user) {
...
Post* StackOverflow::findPost(const std::string& postId) const {
    auto it = std::find_if(posts.begin(), posts.end(), [postId](Post* post) {
...
void StackOverflow::updateUserReputation(const std::string& userId, int points){
    if (User* user = findUser(userId)) user->updateReputation(points);
}
