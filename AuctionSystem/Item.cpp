...
Item::Item(std::string itemId, std::string name, std::string description, ItemCategory category, double startingPrice, std::string sellerId):
            itemId(itemId), name(name), description(description), category(category), startingPrice(startingPrice), 
            sellerId(sellerId), status(ItemStatus::AVAILABLE) {}
...
void Item::displayInfo() const {
    ...
    switch (category){
        case ItemCategory::ELECTONICS: std::cout<< "Electronics"; break; 
        ...
        case ItemCategory::BOOKS: std::cout<< "Collectibles"; break; 
        case ItemCategory::OTHER: std::cout<< "Other"; break; 
    }

    std::cout << std::endl; 

    std::cout << "Starting Price: $" << std::fixed << std::setprecision(2) << startingPrice << std::endl; 
    std::cout << "Status: "; 
    switch(status){
