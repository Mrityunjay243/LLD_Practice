...
Item* AuctionSystem::createItem(std::string sellerId, std::string name, std::string description, ItemCategory category, double startingPrice){
    User *seller = findUser(sellerId); 
    if (!seller || !seller->isActive()) return nullptr; 

    Item* item = new Item(generateItemId(), name, description, category, startingPrice, sellerId); 
    items.push_back(item); 
    seller->addListedItem(item); 
    return item; 
}

Auction* AuctionSystem::createAuction(std::string itemId, std::time_t startTime, std::time_t endTime){
...
bool AuctionSystem::placeBid(std::string auctionId, std::string bidderId, double amount){
    ...
    if (!auction || !bidder || !bidder->isActive() || !auction->isActive() || bidder->getBalance()<amount){
        return false; 
    }
...
Item* AuctionSystem::findItem(const std::string& itemId) const {
    auto it = std::find_if(items.begin(), items.end(), [itemId](Item* item){
