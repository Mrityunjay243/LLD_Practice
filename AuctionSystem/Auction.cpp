...
Auction::Auction(std::string auctionId, Item* item, std::time_t startTime, std::time_t endTime):
...
bool Auction::isActive() const{ return status == AuctionStatus::ACTIVE; }

void Auction::displayInfo() const {
    std::cout<< "\nAuction Details: " << std::endl; 
    std::cout<< "Auction Id: " << std::endl; 
    item->displayInfo(); 
    ...
    if (!bids.empty()){
        ...
        for (const auto& bid: bids){
            std::cout<< "Bidder: " << bid.bidderId
                     << ", Amount: $" << std::fixed << std::setprecision(2) << bid.amount 
