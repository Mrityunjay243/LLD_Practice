...
class HotelManager{
    ...
    public: 
        ...
        Booking* createBooking(std::string guestId, std::string roomNumber, std::string checkInDate, std::string checkOutDate, int numberOfNights); 

        bool checkIn(std::string bookingId); 
        bool checkOut(std::string bookingId); 
        bool cancelBooking(std::string bookingId); 

        void displayAllAvailableRooms() const; 
